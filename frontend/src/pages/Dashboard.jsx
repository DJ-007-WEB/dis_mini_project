...
const Dashboard = () => {
  ...
  const recommendedCoursesList = courses.filter(course => !user.enrolledCourses.includes(course.id)).slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto opacity-0 animate-[fadeIn_0.5s_ease-out_forwards]">
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-1 bg-white rounded-xl shadow-md p-8 border-l-4 border-emerald-600 flex flex-col justify-center transition-all hover:shadow-lg">
            ...
            <p className="text-gray-600">You're making great progress. Ready to continue your learning journey?</p>
          </div>
          ...
          <div className="lg:w-80 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl shadow-md p-6 text-white flex items-center gap-6 relative overflow-hidden transition-all hover:shadow-lg">
            <div className="absolute -right-4 -top-4 text-emerald-500/20 w-32 h-32">
              <svg fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.686 2 6 4.686 6 8c0 2.846 1.954 5.253 4.593 5.864L12 22l1.407-8.136C16.046 13.253 18 10.846 18 8c0-3.314-2.686-6-6-6zm0 14.5l-.5-2.889A4.01 4.01 0 018 10c0-2.206 1.794-4 4-4s4 1.794 4 4a4.01 4.01 0 01-3.5 3.611L12 16.5z"/></svg>
            </div>
            ...
            <div className="z-10">
              <p className="text-emerald-400 text-sm font-semibold uppercase tracking-wider mb-1">Learning Streak</p>
              ...
              <p className="text-xs text-slate-400 mt-1">Best: {stats.bestStreak} days</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 hover:shadow-md hover:border-emerald-300 transition-all group">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Total Hours</p>
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-extrabold text-slate-800 group-hover:text-emerald-600 transition-colors">{stats.totalLearningHours}</span>
                  <span className="text-sm text-gray-500 mb-1">hrs</span>
                </div>
              </div>
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 hover:shadow-md hover:border-emerald-300 transition-all group">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Quiz Average</p>
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-extrabold text-slate-800 group-hover:text-emerald-600 transition-colors">{stats.averageQuizScore}</span>
                  <span className="text-sm text-gray-500 mb-1">%</span>
                </div>
              </div>
              <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 hover:shadow-md hover:border-emerald-300 transition-all group">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Completion Avg</p>
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-extrabold text-slate-800 group-hover:text-emerald-600 transition-colors">{stats.averageCourseCompletion}</span>
                  <span className="text-sm text-gray-500 mb-1">days/course</span>
                </div>
              </div>
            </div>

            {/* Activity Chart */}
            <div className="animate-[fadeIn_0.6s_ease-out_forwards] opacity-0" style={{ animationDelay: '0.1s' }}>
              <ActivityChart data={stats.weeklyActivity} />
            </div>

            {/* Courses In Progress */}
            <div className="animate-[fadeIn_0.6s_ease-out_forwards] opacity-0" style={{ animationDelay: '0.2s' }}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">In Progress</h2>
                <button onClick={() => navigate('/courses')} className="text-sm font-semibold text-emerald-600 hover:text-emerald-700">View All</button>
              </div>
              {enrolledCoursesList.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {enrolledCoursesList.map(course => (
                    <div key={course.id} className="relative group hover:-translate-y-1 transition-transform duration-300">
                      <CourseCard {...course} enrolled={true} />
                      <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/60 to-transparent rounded-t-xl z-10 transition-opacity">
                ...
                <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
                  ...
                  <button onClick={() => navigate('/courses')} className="mt-4 bg-emerald-600 text-white px-6 py-2 rounded-full font-medium hover:bg-emerald-700 transition-colors">Browse Catalog</button>
                </div>
              )}
            </div>

            {/* Recent Activity Feed */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-[fadeIn_0.6s_ease-out_forwards] opacity-0" style={{ animationDelay: '0.3s' }}>
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">Recent Activity</h3>
                <div className="flex gap-2">
                  <button className="px-3 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors focus:ring-2 focus:ring-emerald-500">All</button>
                  <button className="px-3 py-1 text-xs font-medium rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors focus:ring-2 focus:ring-emerald-500">Courses</button>
                </div>
              </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            {/* Upcoming Deadlines */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 animate-[fadeIn_0.6s_ease-out_forwards] opacity-0" style={{ animationDelay: '0.2s' }}>
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-gray-900">Upcoming Deadlines</h3>
                <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-2 py-1 rounded-full">{stats.upcomingDeadlines.length}</span>
              </div>
            ...
            {/* Recommended Courses */}
            <div className="animate-[fadeIn_0.6s_ease-out_forwards] opacity-0" style={{ animationDelay: '0.4s' }}>
